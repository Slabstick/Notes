...
Die Anwendung bietet Zugriff auf eine Datenbank mit Nahrungsmitteln, die detaillierte Informationen zu Kalorien und Makronährstoffen enthält. Unter Makronährstoffen versteht man hauptsächlich Protein, Fett und Kohlenhydrate und sind daher wichtig, da der Anteil der zugenommenen Makronährstoffe über den Tag verteilt, wichtig zur Einhaltung von Fitnesszielen ist. 
NutzerInnen können diese Datenbank nach bereits gespeicherten Nahrungsmitteln durchsuchen, um ihre täglichen Mahlzeiten zusammenzustellen. Zusätzlich können eigene Nahrungsmittel hinzugefügt werden, falls diese nicht vorhanden sind. Jedes Nahrungsmittel kann mit der entsprechenden Menge zum persönlichen Ernährungstagebuch hinzugefügt werden, welches automatisch die Summe der konsumierten Kalorien und Makronährstoffe für den Tag berechnet.

Die Benutzeroberfläche ist darauf ausgelegt, den Ablauf so einfach wie möglich zu gestalten. NutzerInnen können direkt von der Startseite aus auf die Funktionen "Nahrungsmitteldatenbank durchzuchen" und "Ernährungstagebuch anzeigen" zugreifen. Das Tagebuch enthält eine klare und verständliche Zusammenfassung der täglichen Nahrungsaufnahme. Ob es darum geht, Gewicht zu verlieren, Muskelmasse aufzubauen oder einfach eine ausgewogenere Ernährung zu pflegen, die App bietet die Werkzeuge, die NutzerInnen benötigen, um informierte Entscheidungen über ihre Ernährung zu treffen und ihre Gesundheits- und Fitnessziele zu erreichen.

## Projektschnittstellen

### Personenschnittstellen

Die Anforderungen des Projekts stammen von einem Mitarbeiter der QuinScape GmbH, Herrn Florian Hillmann, der hier als Kunde auftritt. Folglich ist Herr Hillmann auch derjenige, dem das Projektergebnis präsentiert und an welchen die Übergabe stattfinden wird. Das langfristige Ziel ist es die Applikation für Smartphones verfügbar zu machen, zunächst aber soll nur ein  Minimal Viable Product (MVP) erstellt werden, der einem ausgewählten Kreis von NutzerInnen zur Verfügung gestellt wird.
...
Im Großen und Ganzen wurde das Projekt dem Antrag entsprechend durchgeführt. Einzige Abweichung findet sich darin, dass es keine dedizierte Startseite gibt, sondern NutzerInnen sofort das Ernährungstagebuch (fortlaufend als "Log" bezeichnet), den zie zuletzt bearbeitet haben oder den Log des aktuellen Datums öffnen. Dies ist mit einer verbesserten UX (User Experience) zu erklären, da der Weg zum Erfassen von Nahrungsmittel deutlich reduziert wird. Sollten in Zukunft noch weitere Funktionen hinzukommen, könnte man über die Implementierung eines so genannten Dashboards nachdenken.

## Technologien

### Java

Für das Backend wurde die Programmiersprache Java in der Version 21 verwendet, da diese die vom Unternehmen meist genutzte Programmiersprache ist.
...
- Von überall in der App sollen NutzerInnen sofort Zugriff auf diese beiden Hauptfunktionen sowie einen Knopf haben, mit dem sie ein Nahrungsmittel in die Datenbank speichern können.
...
2. **View (Ansicht)**:
    ...
    - Die Ansicht ist für die Darstellung der Daten an die BenutzerInnen zuständig. Sie zeigt die Informationen aus dem Modell in einer für die BenutzerInnen verständlichen und ansprechenden Form an. Die Ansicht enthält keine Geschäftslogik, sondern stellt nur das Darstellungslogik bereit.
...
- **Automatische Generierung von CRUD-Methoden**: Spring Data JPA generiert automatisch grundlegende CRUD-Methoden (Create, Read, Update, Delete) für Entitäten, sodass EntwicklerInnen sich nicht um die Implementierung dieser Methoden kümmern müssen.
- **Abfrage-Derivation**: EntwicklerInnen können benutzerdefinierte Abfragen einfach durch das Definieren von Methoden entsprechend benannter Muster in Ihren Repository-Interfaces erstellen.
- **Paging und Sortierung**: Paging und Sortierungen wird durch JPA auf einfache und intuitive Weise unterstützt.
...
Nachdem die Validierung abgeschlossen ist, leitet der Controller die Anfrage an die entsprechenden Service-Methoden weiter. Hier wird die Geschäftslogik angewendet und gegebenenfalls Daten aus der Repository-Schicht abgerufen oder gespeichert. Die Antwort der Service-Schicht wird dann vom Controller entgegengenommen und in ein Format umgewandelt, das für das Frontend verständlich und brauchbar ist – in unserem Fall in JSON, da unser Frontend in TypeScript/JavaScript geschrieben ist und daher nativ das JSON (JavaScript Object Notation) Format unterstützt.
...
Da sich alle Methoden aus den vorangegangenen Methoden der Service-Schicht ableiten, soll hier auf keine Methode im Speziellen eingegangen werden.
...
Zuletzt fehlen noch zwei Komponenten, die den jeweiligen Hauptfunktionen der Applikation untergeordnet sind. Zum einen benötigen wir ein Formular, mit dem sich ein neues Nahrungsmittel in der Datenbank anlegen lässt und mit dessen Hilfe wir bestehende Nahrungsmittel bearbeiten können. Dieses Formular wird dem Feature der Nahrungsmitteldatenbank unterstellt. Andererseits benötigen wir ein weiteres Formular, mit dem sich Nahrungsmittel in ein Log hinzufügen oder dort bereits hinterlegte Nahrungsmittel bearbeiten lassen. Dieses wird folglich dem Feature der Logs untergeordnet.
...
Die Implementierung der Modellschicht verlief ohne Komplikationen. Auch die Entwicklung mit Lombok half dabei auf viel Boilerplate-Code verzichten zu können. Notwendige Teile einer Java-Klasse wie Getter- und Setter-Methoden sowie Konstruktoren müssen so nicht mehr selbst geschrieben werden, sondern werden beim Buildprozess des Backends generiert.

Lediglich in der Verbindung zwischen den Datenbanktabellen kam es zwischenzeitlich zu Herausforderungen bei der Generierung der JSON-Darstellung von Objekten und beim Abruf von Logs mit mehr als einem Eintrag. Im ersten Entwurf war geplant, den Java-Code so zu verfassen, dass die FoodItemEntry-Klasse Zugriff auf die NutritionLog-Klasse und die FoodItem-Klasse bekommt und nur durch Annotationen die IDs hinterlegt sind. Dies führte zu unendlicher Rekursion, da jeder Entry Zugriff auf den NutritionLog bekam und beim Abruf dieses Logs wiederum Zugriff auf die darin liegenden Entries nahm.

Hier hätte man sogenannte DTOs (Data Transfer Objects) implementieren können. Um Zeit zu sparen, konnte das Problem auch umgangen werden, indem der Verweis auf die NutritionLog-Klasse entfernt und stattdessen ein Fremdschlüsselfeld verwendet wurde.

Dies konnte jedoch nur deshalb so leicht umgangen werden, weil das Datenmodell relativ einfach gehalten ist. Mit dem zukünftigen Wachstum der Applikation und der Erweiterung des Datenbankmodells wird der Einsatz von DTOs unerlässlich sein, da diese bei komplexen Backends unverzichtbar sind.

##### Repository

Die Entwicklung des Repositories wurde entsprechend dem Entwurf umgesetzt.

##### Service

Die bedeutendste Änderung im Backendentwurf betrifft die Erstellung eines neuen Logs. Wie erwähnt, soll es für das Frontend unerheblich sein, ob ein Log bereits existiert, wenn ein Eintrag hinzugefügt wird. Da sich dies als komplexer herausstellte als zunächst angenommen, wurde viel Zeit in die Implementierung dieser Funktionalität investiert, um diversen Anforderungen gerecht zu werden. Die Upsert-Service-Methode musste daher mehrmals überarbeitet werden und entsprach nach Ablauf der geplanten Zeit noch nicht den sonst streng befolgten Prinzipien des Clean Code. Nach Abgabe des Projekts wird in diesem Bereich noch Nachbesserungsbedarf bestehen, um das weitere Wachstum und die Wartbarkeit der Anwendung sicherzustellen.

##### Controller

Auch beim Controller konnte der Entwurf weitgehend umgesetzt werden. Allerdings wurde während der Implementierung des Frontends festgestellt, dass die Funktionalität zum Löschen und Ändern von Einträgen in Logs übersehen worden war. Dies führte dazu, dass ein zusätzlicher Endpunkt eingerichtet werden musste, um das Löschen von Einträgen zu ermöglichen.

##### Konfiguration

Ein weiterer Aspekt, der bei der Implementierung und Prüfung des Backends zunächst übersehen wurde und erst auffiel, als die ersten Endpunkte vom Frontend aus angesprochen wurden, war das Fehlen einer CORS-Konfiguration. Um dem Frontend Zugriff auf die API über den konfigurierten Port aus dem Browser zu ermöglichen, musste dieser Port zunächst freigegeben werden. Glücklicherweise war dies dank Spring Boot unkompliziert zu lösen. Es genügte, eine Konfigurationsklasse zu erstellen, diese mit der Annotation @Configuration zu versehen und die Methode addCorsMappings der Klasse WebMvcConfigurer zu überschreiben. Auf diese Weise konnten alle vier erforderlichen HTTP-Methoden für den Port des Frontends freigegeben werden.
...
Um die Fehlersuche beim Programmieren und Ausführen des Codes zu vereinfachen, wurde ein Logger aus der Lombok Bibliothek verwendet. Dieser kann in jeder Klasse hinzugefügt werden, indem dieser die Annotation @Slf4j voranstellt wird. Mit dem Objekt "log" können verschiedene Schweregrade von Meldungen verwendet werden, wie beispielsweise log.info oder log.error (siehe vorherige Abbildung). Falls ein Fehler in einer Backend-Methode auftritt, ist dadurch sofort ersichtlich, wo dieser Fehler entstanden ist.

### Frontend

Die Implementierung des Frontends lief zum größten Teil wie im Entwurf geplant ab. Lediglich den beiden Formularen zur Erstellung und Bearbeitung von Nahrungsmitteln und Logeinträgen wurde ein Abbrechen-Knopf hinzugefügt. Weitere Punkte, die die User Experience verbessern, fielen zwar auf, konnten aber mangels Zeit nicht implementiert werden. Beispielsweise kann im Datumspicker des Formulars zum Hinzufügen von Logeinträgen noch kein Zeitstempel ausgewählt werden. Dies macht das Feld für den Zeitpunkt obsolet und muss nachgeliefert werden. Außerdem müssen den Löschknöpfen noch Abfragen zur Bestätigung des Löschvorgangs hinzugefügt werden, um versehentliches Löschen von Datenbankeinträgen zu verhindern.
...
Zur Vereinfachung der Navigation zwischen den Komponenten wurde auf React Router gesetzt. Dies ist eine Bibliothek, die React erweitert. So können in der Hauptkomponente sogenannte Routes konfiguriert werden, die dann von überall aus in der Applikation angesteuert werden können. Ein weiterer Vorteil ist, dass auch Parameter und Objekte als React Props weitergeleitet werden können. Im Falle dieser Applikation bedeutet das z.B., dass wir eine Route "nutrition-log/:date?" konfigurieren können, welche dann die Möglichkeit bietet, direkt das Datum des gewünschten Logs in der Navigationsleiste aufzurufen. Ruft die App zum Beispiel die URL "nutrition-log/2024-05-27" auf, landet man automatisch im Log des 27.05.2024. Das vereinfacht die Weitergabe von Daten innerhalb der App enorm, da React Props und State nicht mehr manuell durch die einzelnen Komponenten getragen werden müssen.

Ein weiterer Vorteil von React Router ist, dass wir das Location-Interface direkt als Parameter in React Hooks wie useEffect() nutzen können. Dadurch kann z.B. eine useEffect-Funktion geschrieben werden, die die übergeordnete Komponente immer dann neu rendert, sobald mit React Router eine neue URL aufgerufen wird (auch wenn es sich um dieselbe URL handelt, die gerade aufgerufen ist). Ein Nachteil von React besteht darin, dass das Neuladen von Komponenten nicht immer so einfach ist, wie es zunächst erscheinen mag. Löscht man beispielsweise einen Eintrag aus der Datenbank, möchte man aus UX-Perspektive, dass dies auch sofort als Feedback zurückgegeben wird. Dies kann mit React Router sehr einfach realisiert werden, indem man bei jedem Löschvorgang die Komponente neu rendern lässt.
...
Bei der Log Komponente lassen sich einige Abweichungen zum Entwurf erkennen. Neben den Icons, die mit Logos getauscht wurden, gibt es einen zusätzlichen Bereich, in dem die Summen der Kalorien und Makronährstoffe abgelesen werden können. So hat man in jedem Log sofort die gewünschten Daten zur Hand.

Dieser Punkt wurde im Entwurf des Backends eigentlich bedacht, da die Hilfsmethode zur Berechnung der Summen von Anfang an eingeplant war. Ein entsprechender Bereich wurde beim Frontend-Entwurf dann lediglich vergessen. Um die UX zu verbessern, könnten die Zahlen zukünftig noch durch Fortschrittsbalken verziert werden. Zahlen als solche sind weniger intuitiv und sollten mit Fortschrittsbalken versehen werden, an denen man sofort erkennen kann, wie nah man sich an den täglichen Grenzen oder Zielen befindet.
...
Bei diesem Formular gibt es eine kleine Abweichung vom Entwurf. Statt "Entry" steht in der Überschrift der Name des Nahrungsmittels, das man zum Log hinzufügen möchte. Dies hilft den NutzerInnen zu sehen, dass das richtige Nahrungsmittel ausgewählt wurde.

Gerade die Implementierung der Formulare hat im Bereich des Frontends die meiste Zeit in Anspruch genommen. Hier wurde nach der Dokumentation der Komponentenbibliothek shadcn/ui gearbeitet, wodurch noch weitere Bibliotheken herangezogen werden mussten. Formulare sind im Internet zwar eine der gebräuchlichsten Funktionalitäten, aber gerade in Kombination mit React stellen sie eine der größten Hürden dar. Dies hat einerseits den Hintergrund, dass die gebräuchlichsten Angriffe auf die Sicherheit einer Seite über Formulare erfolgen, und andererseits werden Formulare in React "kontrolliert" realisiert. Das bedeutet, dass jede Änderung des Inhalts eines Formularfelds ein Event aufruft, im Gegensatz zum gebräuchlichen Weg, bei dem ein Event erst aufgerufen wird, wenn die NutzerInnen auf den Senden-Knopf drücken.

Die Formularbibliothek ZOD versucht dies einerseits zu vereinfachen und bietet zudem die Funktionalität, Formularfelder direkt bei der Eingabe auf korrekte Daten zu überprüfen. In diesem Fall wurde sichergestellt, dass Zahlenfelder ausschließlich numerische Eingaben akzeptieren und diese Zahlen positiv sein müssen. Das Kalorienfeld muss mindestens den Wert 1 aufweisen, und das Unit-Feld darf lediglich zwei unterschiedliche Zustände annehmen (Gramm oder Milliliter). Dies erfordert jedoch die Erstellung zusätzlicher Datentypen, Interfaces und Schemata, um die Typensicherheit im Hintergrund reibungslos zu gewährleisten.

## Testphase

Aus der Projektplanung ist ersichtlich, dass ein Großteil der Testphase parallel zur Implementierungsphase stattfand. Insbesondere im Fall des Backends wäre es wenig sinnvoll gewesen, zunächst alle Endpunkte zu erstellen und diese anschließend gebündelt zu testen. Obwohl das Backend schichtweise strukturiert ist, erfolgte die Implementierung dennoch nach einzelnen Features. Dies ermöglichte es, jedes abgeschlossene Feature unmittelbar zu testen. Zur Überprüfung des Backends wurde das Tool Postman verwendet.
...
Bei der Durchführung des Projekts fiel vor allem auf, wie schwierig es ist, ein Projekt eigenständig zu planen und sich an diese Planung zu halten. Hierbei waren vor allem die täglichen Sprint-Meetings eine große Hilfe, um die Arbeit des vergangenen Tages Revue passieren zu lassen und die Arbeit der folgenden 8 Stunden organisiert zu strukturieren. Eine vollständige Applikation zu entwickeln, inklusive Backend und Frontend, war eine wichtige Erfahrung auf dem Weg, ein vollwertiges Mitglied in zukünftigen EntwicklerInnenteams zu werden.
